...
class DatumLegendWithMeasuresRequest extends StatelessWidget {
  final List<charts.Series> seriesList;
  final bool animate;

  DatumLegendWithMeasuresRequest(this.seriesList, {this.animate});

  factory DatumLegendWithMeasuresRequest.withSampleData() {
    return new DatumLegendWithMeasuresRequest(
      _createSampleData(),
  ...
  factory DatumLegendWithMeasuresRequest.withRandomData() {
    return new DatumLegendWithMeasuresRequest(_createRandomData());
  }

  /// Create random data.
  static List<charts.Series<LinearSales, dynamic>> _createRandomData() {
    final random = new Random();

    final data = [
      new LinearSales("online R พ่อง", random.nextInt(100)),
      new LinearSales("walkin R แม่", random.nextInt(100)),
    ...
    return new charts.PieChart(
      ...
      behaviors: [
        ...
        new charts.DatumLegend(
          ...
          cellPadding: new EdgeInsets.only(right: 0, bottom: 0),
...
//
/// Also shows the option to provide a custom measure formatter.
class DatumLegendWithMeasuresQueue extends StatelessWidget {
  final List<charts.Series> seriesList;
  final bool animate;

  DatumLegendWithMeasuresQueue(this.seriesList, {this.animate});

  factory DatumLegendWithMeasuresQueue.withSampleData() {
    return new DatumLegendWithMeasuresQueue(
      _createSampleData(),
      // Disable animations for image tests.
      animate: false,
    );
  ...
  // It is used for creating random series data to demonstrate animation in
  // the example app only.
  factory DatumLegendWithMeasuresQueue.withRandomData() {
    return new DatumLegendWithMeasuresQueue(_createRandomData());
  }
